...
/**
 * Lavender products from the Square catalog (the "Online Sales | Lavender Shop"
 * category). Shared by the /api/products/lavender route and server components
 * such as the homepage teaser. Throws on Square errors; callers decide how to
 * degrade.
 */
...
export async function getLavenderProducts(): Promise<Product[]> {
  {
    ...
    const products = allItems
      .map((item: SquareCatalogItem) => {
        ...
        // Display category: the first Square category that is not the
        // "Online Sales | Lavender Shop" gate. That category only marks what
        // is sold online; it is not a shelf and never shows as a filter or
        // label. Products with no other category get none.
        let categoryName: string | undefined;
        for (const c of itemData.categories ?? []) {
          if (!c.id || c.id === lavenderCategoryId) continue;
          const name = categoryMap.get(c.id);
          if (name) {
            categoryName = name;
            break;
          }
